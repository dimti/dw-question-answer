...
if ( dwqa_current_user_can( 'post_answer' ) ) {
	...
	<p>
		<?php
		printf( '%1$s <a href="%2$s" onclick="event.preventDefault();jQuery(\'#redirect_to\').val(\'' . $redirect . '\');jQuery(\'#modal-registration\').modal(\'show\');" title="%3$s">%3$s</a>', 'Если у вас еще нет аккаунта на нашем сайте, вам надо', wp_registration_url(), 'зарегистрироваться' );
		?>
	</p>
	<?php
	}
